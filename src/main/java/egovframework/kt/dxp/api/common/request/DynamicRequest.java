...
public record DynamicRequest(
    ...
    public String getFieldValue(String fieldName) {
        if (StringUtils.isEmpty(fieldName)) {
            throw new ServiceException(ErrorCode.INVALID_PARAMETER, "'fieldName' can't be empty.");
        }
        return this.filter.stream()
                .filter(filter -> fieldName.equals(filter.field()))
                .map(DynamicFilter::value)
                .findFirst()
                .orElseThrow(() -> new ServiceException(ErrorCode.INVALID_PARAMETER, fieldName + " value doesn't exist."));
    }
