package egovframework.kt.dxp.api.common.jpa.querydsl.repository;
...
import egovframework.kt.dxp.api.common.jpa.querydsl.DynamicBooleanBuilder;
...
public class JpaDynamicDslRepositoryImpl<T, ID extends Serializable> extends SimpleJpaRepository<T, ID> implements JpaDynamicRepository<T, ID> {
    ...
    private void checkNamedEntityGraph(JPAQuery<T> query) {
        ...
        if (StringUtils.isNotEmpty(namedEntityGraphName)) {
            String hintName = "jakarta.persistence.loadgraph";
            query.setHint(hintName, this.entityManager.getEntityGraph(namedEntityGraphName));
        }
    }
}
